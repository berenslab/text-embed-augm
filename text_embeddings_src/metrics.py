...
def knn_acc(
    ...
    if test_embeddings is None:
        X_train, X_test, y_train, y_test = train_test_split(
            embeddings, labels, test_size=test_size, random_state=rs
        )
    else:
    ...
    if not isinstance(embeddings, list):
        ...
        if test_embeddings is not None:
            assert len(embeddings) == len(
                test_embeddings
            ), "Train and test splits don't have the same number of representations to evaluate."

    knn_accuracy_values = []
    for i, embed in enumerate(embeddings):
        knn_accuracy_values.append(
            knn_acc(
                embed,
                labels,
                test_embeddings=test_embeddings[i],
...
def lin_acc(
    embeddings, labels, test_embeddings=None, test_labels=None, test_size=0.1, rs=42
):
